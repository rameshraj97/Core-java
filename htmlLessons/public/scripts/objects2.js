...
class RegisterNumber {
    #number; //Private field 
    // # is legal inside javascript class but illegal in javascript objects
    constructor() {
        this.#number = 1;
    }
    getNumber() { return this.#number; }
    #printToScreen() { return this.#number; } // private method
    callPrint() {
        const x = this.#printToScreen(); // call a private method with in a same class 
        return this.x;
    }
}
const rn = new RegisterNumber();
console.log(Next Number ${rn.getNumber()});
console.log(Private Number ${rn.callPrint()});
